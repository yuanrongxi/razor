...
static void test_probing(int use_delay_based)
{
	...
	sender_estimation_update_block(est, 0, 50, 1, now_ms);

	if (use_delay_based == 0)
		sender_estimation_update_delay_base(est, now_ms, remb_bps);
	else
		sender_estimation_update_remb(est, now_ms, remb_bps);
	sender_estimation_update(est, now_ms);
	...
	if (use_delay_based == 0)
		sender_estimation_update_delay_base(est, now_ms, second_remb_bps);
	else
...
static void bitrate_decrease_without_following_remb()
{
	...
	/*模拟report事件,发生丢包了*/
	sender_estimation_update_block(est, k_fraction_loss, k_rtt, 100, now_ms);
	now_ms += 1000;
	sender_estimation_update(est, now_ms);
